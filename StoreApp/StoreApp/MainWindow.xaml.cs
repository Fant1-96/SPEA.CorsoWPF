...
namespace StoreApp
{
    public partial class MainWindow : Window
    {
        ...
        private void LoadReturns() { ReturnsGrid.ItemsSource = _context.Returns.Include(r => r.Order.User).Include(r => r.ReturnedProduct).ThenInclude(op => op.Product).AsNoTracking().ToList(); }
        ...
        private void DeleteUser_Click(object sender, RoutedEventArgs e) { var user = (ApplicationUser)UsersGrid.SelectedItem; if (user == null) return; if (MessageBox.Show("Confermi cancellazione?", "Attenzione", MessageBoxButton.YesNo) == MessageBoxResult.Yes) { _context.Users.Remove(user); _context.SaveChanges(); LoadUsers(); LoadOrders(); LoadReturns(); } }
        ...
        private void DeleteOrder_Click(object sender, RoutedEventArgs e) { var order = (Order)OrdersGrid.SelectedItem; if (order == null) return; if (MessageBox.Show("Confermi cancellazione?", "Attenzione", MessageBoxButton.YesNo) == MessageBoxResult.Yes) { _context.Orders.Remove(order); _context.SaveChanges(); LoadOrders(); LoadReturns(); } }
